        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    // [MODIFIED] Bind descriptorset with font or user texture
                    VkDescriptorSet descSet[1] = { (VkDescriptorSet) pcmd->TextureId };
                	vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, descSet, 0, NULL);
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // [MODIFIED] Store our identifier
    io.Fonts->SetTexID(bd->DescriptorSet);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    // Create the Render Pass
    {
        ...
        //Imui_ImplVulkan_CreatePipeline(device, allocator, VK_NULL_HANDLE, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, bd->Subpass);
    }
    ...
    // Create The Image Views
    {
        ...
        info.components.r = VK_COMPONENT_SWIZZLE_IDENTITY; // [MODIFIED] only supports swizzle identity
        info.components.g = VK_COMPONENT_SWIZZLE_IDENTITY; // [MODIFIED] only supports swizzle identity
        info.components.b = VK_COMPONENT_SWIZZLE_IDENTITY; // [MODIFIED] only supports swizzle identity
        info.components.a = VK_COMPONENT_SWIZZLE_IDENTITY; // [MODIFIED] only supports swizzle identity
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    #else
    slim::SwapchainSupportDetails swapchainSupport = slim::QuerySwapchainSupport(v->PhysicalDevice, wd->Surface);

    // [MODIFIED] Select Surface Format (to align swapchain surface format)
    wd->SurfaceFormat = slim::ChooseSwapSurfaceFormat(swapchainSupport.formats);

    // [MODIFIED] Select Present Mode (to align swapchain surface present mode)
    wd->PresentMode = slim::ChooseSwapPresentMode(swapchainSupport.presentModes);
    #endif
